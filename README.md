...
- Dataset ParentA (user scanner Traverse permission)
...
I have a remote server at a buddy that has been kernel panicking for the last 12 months. This server has proxmox as a hypervisor running two VM's. The first is an opnsense firewall with a netword card pcie passthrough while the second one is a windows vm with remote desktop feature for me to login to in case the vpn from the opnsense instance is down. The server at boot, turns on both vm instances. The opnsense firewall serves me as a site to site wireguard vpn connection. This allows me to securely access my second server which is a TrueNas server at my buddy as well. So for the last 12 months, the server would freeze if the windows machine was left turned on. After manyu unsuccessful remote troubleshooting, I decided to leave it as is and just make sure to turn off the Windows VM when not needed. However, something funny one day happened, the opnense vm decided to swith the lan ports assignments. Meaning that the wan and lan ports were swapped. This caused an issue at my buddy because instead of having only one PPPOE connection inside his lan, he suddenly had two dhcp servers trying to assign IP addresses inside his network. A couple of remote troubleshooting led me to discover this and we temporarly had to switch the lan cables. My impression is that there is some issue at the hardware level where the nic dissapeared for a second that lead the firewall to reinitialise the network interfaces.
